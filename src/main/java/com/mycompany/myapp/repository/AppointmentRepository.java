...
public interface AppointmentRepository extends JpaRepository<Appointment, Long>, JpaSpecificationExecutor<Appointment> {
    @Query("select appointment from Appointment appointment where appointment.owner.login = ?#{authentication.name}")
    ...
    @Query(
        value = "select appointment from Appointment appointment left join fetch appointment.owner",
    ...
    @Query("select appointment from Appointment appointment left join fetch appointment.owner")
    List<Appointment> findAllWithToOneRelationships();

    @Query("select appointment from Appointment appointment left join fetch appointment.owner where appointment.id =:id")
