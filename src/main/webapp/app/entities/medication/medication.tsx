        ...
        {medicationList && medicationList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="healthyMeApp.medication.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="healthyMeApp.medication.owner">Owner</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {medicationList.map((medication, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/medication/${medication.id}`} color="link" size="sm">
                      {medication.id}
                    </Button>
                  </td>
                  <td>{medication.name}</td>
                  <td>{medication.rating}</td>
                  <td>{medication.notes}</td>
                  <td>{medication.owner ? medication.owner.login : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/medication/${medication.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        tag={Link}
                        to={`/medication/${medication.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/medication/${medication.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="healthyMeApp.medication.home.notFound">No Medications found</Translate>
            </div>
          )
        )}
      </div>
      {totalItems ? (
        <div className={medicationList && medicationList.length > 0 ? '' : 'd-none'}>
