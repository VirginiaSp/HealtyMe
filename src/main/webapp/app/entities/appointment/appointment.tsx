  ...
  return (
    <div>
      <h2 id="appointment-heading" data-cy="AppointmentHeading">
        <Translate contentKey="healthyMeApp.appointment.home.title">Appointments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="healthyMeApp.appointment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/appointment/new" className="btn btn-primary create-entity-btn" id="create-entity-btn" data-cy="entityCreateButton">
        ...
        {appointmentList && appointmentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="healthyMeApp.appointment.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('specialtySnapshot')}>
                  <Translate contentKey="healthyMeApp.appointment.specialtySnapshot">Specialty Snapshot</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('specialtySnapshot')} />
                </th>
                ...
                <th>
                  <Translate contentKey="healthyMeApp.appointment.owner">Owner</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="healthyMeApp.appointment.doctor">Doctor</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
                ...
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {appointment.startDateTime ? (
                      <TextFormat type="date" value={appointment.startDateTime} format={APP_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>{appointment.durationMinutes}</td>
                  <td>{appointment.address}</td>
                  <td>{appointment.specialtySnapshot}</td>
                  <td>{appointment.notes}</td>
                  <td>
                    <Translate contentKey={`healthyMeApp.AppointmentStatus.${appointment.status}`} />
                  </td>
                  <td>{appointment.owner ? appointment.owner.login : ''}</td>
                  <td>{appointment.doctor ? <Link to={`/doctor/${appointment.doctor.id}`}>{appointment.doctor.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="healthyMeApp.appointment.home.notFound">No Appointments found</Translate>
            </div>
          )
        )}
      </div>
